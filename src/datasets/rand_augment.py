...
LEVEL_TO_ARG = {
    ...
    "PosterizeOriginal": _posterize_original_level_to_arg,  # 变换的几种变体;
...
class AugmentOp:
    ...
    def __init__(self, name, prob=0.5, magnitude=10, hparams=None):
        ...
        self.hparams = (
            hparams.copy()
        )  # {'translate_const': 144, 'interpolation': 3, 'magnitude_std': 0.5}

        self.kwargs = {
            "fillcolor": hparams["img_mean"]
            if "img_mean" in hparams
            else _FILL,  # (128, 128, 128) ; 
            "resample": hparams["interpolation"]  # 3 ;
        ...
        self.magnitude_std = self.hparams.get(
            "magnitude_std", 0
        ) 

    def __call__(self, img_list):
        if (
            self.prob < 1.0 and random.random() > self.prob
        ):
            return img_list
        magnitude = self.magnitude  # 7 the defined the augmentation magnitude;
        ...
        if isinstance(img_list, list): 
            ...
            return self.aug_fn(
                img_list, *level_args, **self.kwargs
            )
...
def rand_augment_transform(
    ...
    sections, not order sepecific determine
        ...
        'n' - integer num layers (number of transform ops selected per image) # 总共有一堆变换方式, 定义每幅图片所选择的增强的方式;
    ...
    config = config[1:]  # ['m7', 'n4', 'mstd0.5', 'inc1']

    for c in config:
        cs = re.split(r"(\d.*)", c)  # ['m', '7', '']
    ...
    return RandAugment(
        ra_ops, num_layers, choice_weights=choice_weights
    )  
