...
def one_hot_modify(targets, num_classes, on_value=1.0, off_value=0.0):
    """
    This function converts target class indices to one-hot vectors, given the
    number of classes.
    Args:
    ...
    return torch.where(targets==1, on_value, off_value)
...
def mixup_target(target, num_classes, lam=1.0, smoothing=0.0):
    ...
    """
    off_value = smoothing / num_classes # 平均数, 代表随机乱猜; 
    on_value = 1.0 - smoothing + off_value # 多标签的独立性, 应该是可以直接饮用多标签分类任务的; 
    
    target1 = one_hot_modify(
        target,
        num_classes,
        on_value=on_value,
        off_value=off_value,
    )
    target2 = one_hot_modify( 
        target.flip(0),
        num_classes,
        on_value=on_value,
        off_value=off_value,
...
class MixUp:
    ...
    def __init__(
        ...
        self.label_smoothing = label_smoothing # Apply Label smoothing; 
    ...
    def _get_mixup_params(self):
        ...
        if np.random.rand() < self.mix_prob: # don't apply it;  
            if self.mixup_alpha > 0.0 and self.cutmix_alpha > 0.0:  # basic value check! 
    ...
    def _mix_batch(self, x):
        lam, use_cutmix = self._get_mixup_params() # torch.Size([2, 3, 48, 224, 224]) all images in single batch; 
        if lam == 1.0: 
            return 1.0
