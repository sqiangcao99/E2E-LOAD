...
def is_eval_epoch(cfg, cur_epoch, multigrid_schedule):
    ...
    """
    # pdb.set_trace()
    if (cur_epoch + 1 == cfg.SOLVER.MAX_EPOCH) and (cur_epoch + 1 >= cfg.TRAIN.EVAL_START_EPOCH):
        return True
    ...
    return ((cur_epoch + 1) % cfg.TRAIN.EVAL_PERIOD == 0) and (cur_epoch + 1 >= cfg.TRAIN.EVAL_START_EPOCH)
...
def aggregate_sub_bn_stats(module):
    ...
    for child in module.children():
        if isinstance(child, SubBatchNorm3d): # 每个Batch会分成若干个Group, 因此每个模块会有多个参数估计, 因此在评估之前需要进行同步; 
...
#     rt = tensor.clone()
#     du.all_reduce(rt, op=dist.ReduceOp.SUM)
#     rt /= n
#     return rt
